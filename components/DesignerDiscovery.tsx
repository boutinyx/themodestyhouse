...
export default function DesignerDiscovery({
  ...
  // pb split out of the old `py-10 md:py-20` pair, 2026-08-26 — Tina: "make
  // space between map & guide smaller", then "can i get the spacing back on
  // mobile tho". So the cut is DESKTOP ONLY: 80px -> 40px there, 40px kept on a
  // phone. `pb-10` with no `md:` variant is what expresses that — one value at
  // every width, where the top padding still doubles at md.
  // The seam is this section's BOTTOM padding plus the editorial section's TOP
  // padding, so both sides moved together; see the matching note in
  // app/page.tsx. The top padding is untouched — the gap above the map was not
  // the complaint.
  //
  // A JS comment, not a JSX one: `return (` takes a single element, so a
  // `{/* */}` here is a second child and the file will not parse.
  return (
    <section className="max-w-[1220px] mx-auto px-8 pt-10 md:pt-20 pb-10">
