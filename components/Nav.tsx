...
export function Nav() {
  ...
  const categoryItems: NavItem[] = CATEGORY_LANES.map((l) => ({
    ...
    // A row with subItems ALSO navigates to its own bare href on a mouse
    // click now (NavMenu.tsx) — added 2026-08-15, after Tina: "i cant click
    // on hijabs and scarfes now and also not outterwear", then "all i
    // wanted you to do is make it able to click on this and being able to
    // open all" when a same-evening first attempt added a separate "All X"
    // flyout entry instead of making the row itself clickable. That first
    // attempt is gone — see NavMenu.tsx for how the row itself now handles
    // both hover-opens-flyout AND mouse-click-navigates without the touch
    // race that made this row a non-Link Menu.Trigger in the first place.
    ...(l.slug === 'outerwear'
      ? {
