...
| 1.3 | 5 title tags too long | Measured across all 160 sitemap pages: **>60: 25 · >65: 19 · >70: 15 · >75: 5**. Their "5" is the **>75-character** count — the only threshold in range that gives exactly 5 (>74 gives 7, >76 gives 2; no pixel threshold gives 5 either). The five: `/modest-swimwear` 79, `/editorial/still-boiling-feeling-fall` 79, `/edits/fall-essentials` 76, `/editorial/where-to-buy-hijabs-online` 76, `/editorial/best-abaya-brands-price-tiers` 76. Systematic cause: every title carries the 20-character ` \| The Modesty House` suffix | **Closed 2026-09-15.** Tina asked for it directly, and no headline was rewritten: `TITLE_MAX` went 75 -> 60, so the 20-character ` \| The Modesty House` tail drops wherever it would push a title over. Every one of the 25 is 41-59 characters on its own. Brand pages go through the same helper. Verified on staging: **0 of 160 pages over 60**, longest now 59 | **done** |
...
- ~~**Two hand-written descriptions are still over**~~ — **closed 2026-09-15**, at Tina's
  request. `/editorial/where-to-buy-hijabs-online` was 192: the visible dek on the page is
  UNCHANGED and a 146-character `seoDescription` was added to the frontmatter instead, which is
  exactly what that field exists for (`lib/posts.ts::seo`) — her own sentence, stopped one
  clause earlier. `/edits/fall-essentials` went 175 -> 155 by ending its colour list at olive.
  Verified on staging: **0 of 160 pages over 160 characters**.
