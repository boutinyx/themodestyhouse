...
export const LANE_ANSWERS: Record<string, LaneAnswer> = {
  ...
  'modest-abayas': {
    h2: 'What is an abaya?',
    body: "An abaya is a loose, full-length outer garment, traditionally worn as a robe over regular clothing rather than as the outfit itself. Open-front abayas fasten like a coat and layer over a dress or trousers; closed abayas pull on as one piece. Kimono and butterfly cuts use wide, dropped sleeves for a more relaxed drape, while a fitted or 'sharp' abaya follows the body more closely through the shoulders. Black is the traditional colour, but modern abayas range across the full spectrum, from plain and minimal to heavily embellished for occasion wear. Because it's an outer layer, an abaya is usually sized to fit over whatever's worn underneath, not to size. Regional cuts vary too — an Emirati abaya tends to be closer-fitted through the shoulder than a Moroccan or South Asian one, and most are best cared for with a delicate wash or dry clean rather than a regular machine cycle. Fabric is what the cut is made in, and it changes the season more than the shape does: across the abayas listed here, satin, linen, chiffon and crepe are the four that recur, with linen and chiffon reading as summer weights and crepe and satin holding structure in colder months. The vocabulary overlaps with neighbouring garments and is worth separating. A kaftan is cut wider and shorter through the body and is often worn as the outfit rather than over one. A jilbab covers head and body together, so it is not layered the same way. A bisht is a lighter open robe, and a khimar is a head covering, not an abaya at all. Because an abaya is an outer layer, what goes underneath decides how it hangs — an under-dress or a base layer keeps a lighter fabric from clinging to whatever is beneath it. Price varies more by where a house is than by how the piece is made: a British or American abaya commonly sits under 100 in its own currency, while houses in Dubai and Istanbul price several times higher for comparable cuts. Telling a well-made abaya from a cheap one is mostly a matter of where you look. Check the shoulder first: on a good one the seam sits where your shoulder actually ends and the fabric falls straight from it, while a rushed cut drops the seam down the arm and the whole garment swings forward. Then the hem and the sleeve edges — a double-folded, evenly stitched hem holds its line for years; a single overlocked edge starts to ripple after a few washes. Weight tells you the rest. Hold the fabric up to a window: a well-chosen crepe or satin stays opaque under strong light, and if it doesn't, the piece needs a lining it probably hasn't got. Embellishment is the least reliable signal of all, because beading is cheap to add and expensive to do properly — look at whether it's stitched or glued before reading it as a sign of quality.",
    related: ['modest-dresses', 'modest-hijabs'],
  },
  'modest-hijabs': {
    h2: 'What fabric should I choose for a hijab?',
    body: "Fabric changes how a hijab wears more than colour or print does. Chiffon is lightweight and slightly sheer, drapes well for occasion wear, but usually needs an underscarf to stop it slipping. Jersey is stretchy and grips on its own, which makes it the easiest fabric for everyday wear and sport. Satin and silk have a formal sheen and a smooth hand, but the same smoothness makes them prone to sliding, so they're pinned rather than tucked. Crinkle or crepe fabrics hold texture and shape without ironing, which is why they've become a popular middle ground between jersey's grip and chiffon's drape. Shape matters too: a square hijab folds into a triangle for a classic wrap, while a long rectangular shayla wraps and pins with less folding, which is why most everyday hijabs are cut rectangular rather than square. Two fabrics dominate what is actually listed here and one of them is often left out of these explanations: jersey first, then modal, which drapes softer than jersey while keeping enough grip to wear without pins — the reason it has become an everyday default for many people. Underneath matters as much as the scarf. An undercap or underscarf is a close-fitting cotton or jersey cap worn first: it holds hair back, gives a slippery fabric something to sit against, and is what makes chiffon and satin wearable at all. Instant or slip-on hijabs are sewn into shape already, so they pull on without wrapping or pinning, which makes them the easiest starting point. Size decides the wrap more than style does — a longer rectangle allows a second pass around the neck, a shorter one does not. Care follows the fabric: jersey and modal wash normally, chiffon and satin prefer cold water and air drying, and crinkle should never be ironed flat. Quality in a hijab comes down to the edge and the weight. Look at the hem: a good scarf is finished with a narrow rolled or double-stitched edge that lies flat, while a cheap one is cut and overlocked, which shows as a hard ridge through the drape and unravels first. Weight matters more than fibre name — the same fabric can be woven light enough to be see-through or dense enough to sit properly, and the denser one drapes better and holds colour after washing. Pilling is the giveaway over time: jersey and modal that pill within a few wears were spun short-staple, whatever the label said. And check the colour against daylight rather than a screen, because thin dye is the easiest corner to cut and the hardest to photograph.",
