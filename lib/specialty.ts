...
// flagged three examples (BNAH's sleeveless mock-neck "singlet" top,
// ilovemodesty's "Black Neck Cover") that were scattered across dress/set/top
// because nothing in the classifier distinguished them. Vocabulary below is
// every phrase actually found in the raw catalogue for this concept — 55 rows
// across 11 brands, checked 2026-08-11 — not a guessed pattern; each term is
// unambiguous on its own (a "dickey" or "neck cover" is never a complete
// outfit). `\bbase layer\b` excludes a trailing "dress"/"abaya" because a
// full-length base-layer DRESS (e.g. mariams' MS157/MS190) is still a
// complete, standalone garment — only the accessory-scale pieces belong here.
// Deliberately excludes hijab/underscarf/bonnet titles: those stay in Hijabs
// per Tina's call, even when they also happen to cover the neck.
const LAYERING_RE = /\bneck cover\b|\bdicke?y\b|\bmodesty panel\b|\bbase layer\b(?!\s+(?:abaya\s+)?dress)|\bshoulder.?cover\b|\bsleeve (?:cover|extender|add.?on)s?\b|\barm sleeves?\b|\bcollar (?:cover|insert)\b|\binner top\b|\bbody top\b|\bsinglet\b/i;
