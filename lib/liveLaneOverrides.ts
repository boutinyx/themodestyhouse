 ...
 *  have been merged into data/lane-overrides.json via
 *  scripts/merge-live-edits.mjs. Same atomic write as setLiveLaneOverride,
 *  so a concurrent read never sees a truncated file. */
export function clearLiveLaneOverrides(storePath: string = STORE_PATH): void {
  mkdirSync(path.dirname(storePath), { recursive: true });
  const tmp = `${storePath}.${process.pid}.tmp`;
