...
- **Your saved favourites.** If you save an item, your list is stored **only in your own browser** (in local storage, under the key `tmh_favs`). The list itself never reaches our servers, is not linked to you, and is cleared if you clear your browser data. Each individual save is counted anonymously — see *Pieces you save*, below.
...
- **Page views (a second analytics tool, for comparison).** Alongside Pulse, we also run **Plausible**, an open-source analytics tool, so we can compare the two. Unlike Pulse, we run it ourselves — it is **self-hosted on our own infrastructure** rather than operated by a separate company, so this page-view data never reaches a third party. It collects the same kind of anonymous data as Pulse: which pages are viewed, the referring site, device type, browser, operating system, country and screen size. It sets **no cookies**, and your IP address is used only momentarily, to work out your country, and is never stored.
...
- **Buttons and links you tap, in general.** Alongside the specific actions listed above, Pulse records that a click happened on a button or link and **the words written on it** — for example “Load more” or the name of a lane. The label is shortened to 60 characters, and any email address or long run of digits in it is replaced with `[email]` or `[number]` **in your browser, before anything is sent**, so a label that happens to contain either never leaves your device intact. In the same way we record **that** something was copied from a page and **how much** — never what was copied — and, when a form is sent, **the form's name and how many fields it had** — never a value you typed and never a field's name. We do this so we can see which parts of the site people actually use rather than guessing. As with everything above, it uses no cookies and no identifier for you, and none of it is recorded if your browser sends a Do Not Track or Global Privacy Control signal.
...
- **Measuring how the site is used** (Pulse analytics, and Plausible, our self-hosted comparison tool) — to understand which pages are useful. Because neither uses cookies nor stores an identifier for you, we rely on legitimate interest, Art. 6(1)(f). If we ever add analytics that identify you or follow you across sites, we will ask for your consent first.
...
- **Pulse (ciphera.net)** — our analytics provider. It receives the anonymous page-view information described in Section 2. Its script and its event endpoint are delivered through a European content network (Amsterdam); see Section 7. For more information, see [Pulse's documentation](https://pulse.ciphera.net/faq).
- **Plausible** — not a separate company we share data with. It is open-source software we run ourselves, on our own Railway hosting (already listed below), in the same European (Amsterdam) region as the rest of the site. The page-view data it collects, described in Section 2, never leaves our own infrastructure.
...
- **Analytics:** raw event data is automatically deleted by Pulse after **6 months**; aggregated statistics may be kept longer. Because no cookie or persistent identifier is used, none of it is linked to you as an individual. Our self-hosted Plausible instance is a comparison tool we control directly; we intend to keep it running only for as long as that comparison is useful, and to delete its data when we retire it.
...
To exercise any of these, email **hello@themodestyhouse.com**. We will respond within one month.

You also have the right to complain to the Dutch data protection authority, **Autoriteit Persoonsgegevens** — [autoriteitpersoonsgegevens.nl](https://autoriteitpersoonsgegevens.nl).
...
**The Modesty House** — [hello@themodestyhouse.com](mailto:hello@themodestyhouse.com)
